...
describe('Simple NFT', function () {
  ...
  it('Does anyone can update contract uri? (should not)', async function () {
    ...
    expect(await contractURI_final).to.equal('Test New ContractURI');
  });
  ...
  it('Does contract owner can update base uri? (should be)', async function () {
    ...
    expect(contractURI_final).to.equal('Test New ContractURI')
  });
  ...
  it('Does anyone can update base uri? (should not)', async function () {
    ...
    await contract.connect(user1).setBaseURI('Test');

    const baseURI_final = await contract.baseURI();
    console.log('baseURI', baseURI_final);

    expect(baseURI_final).to.equal('Test')
  });
  ...
  it('Does contract owner can update contract uri and base uri? (should be)', async function () {
    ...
    expect(contractURI_final).to.equal('Test New ContractURI');
    expect(baseURI_final).to.equal('Test New BaseURI');
  ...
  it('Does anyone can update contract uri and base uri? (should not)', async function () {
    ...
    await contract.connect(user1).setContractURI('Test New ContractURI');
    await contract.connect(user1).setBaseURI('Test New BaseURI');

    const contractURI_final = await contract.contractURI();
    const baseURI_final = await contract.baseURI();

    expect(contractURI_final).to.equal(contractURI);
    expect(baseURI_final).to.equal(baseURI);
  });
