...
const setupContract = async (addAddressesToOG = [], addAddressesToWL = [], maxSupply = constants.MAX_SUPPLY) => {
    // Get contract artifacts
    const ContractFactory = await ethers.getContractFactory(constants.CONTRACT_NAME);

    const WHITELIST_root = await functions.buildRoot([...constants.WhiteList, ...addAddressesToWL]);

    // Deploy contracts
    const contract = null
    
    // To-Do: Add the contract deployment code here
